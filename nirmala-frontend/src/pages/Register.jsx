...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setErrorMsg(error.response?.data?.error || "Gagal melakukan registrasi");
    }
  };

  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans text-slate-800">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 border-t-4 border-emerald-500 my-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2 text-sm font-medium">Daftar untuk mulai kelola sampahmu</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Nomor Telepon / WhatsApp</label>
            <input 
              type="tel" 
              className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 text-slate-800 focus:bg-white focus:outline-none focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all duration-200"          
          ...
          <div>
            ...
            <textarea 
              className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 text-slate-800 focus:bg-white focus:outline-none focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all duration-200 resize-none"          
              placeholder="Masukkan alamat rumahmu"
              rows="2"
              value={alamat}
          ...
          <button 
            type="submit" 
            className="w-full bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white font-bold py-3.5 px-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 mt-6"
          >
            Daftar Sekarang
          </button>
        </form>
